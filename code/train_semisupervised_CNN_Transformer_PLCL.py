...
# CL
parser.add_argument('--CL_type', type=str, default='both', help='CL implement type: both, prune_by_class, prune_by_noise_rate')
parser.add_argument('--weak_weight', type=float, default=5.0, help='weak_weight')
parser.add_argument('--iter_plstart', type=float, default=400, help='iter_num to start for PL')
parser.add_argument('--iter_clstart', type=float, default=200, help='iter_num to start for CL')
parser.add_argument('--labeled_num', type=int, default=20, help='labeled number')
...
if __name__ == '__main__':
    ...
    labeled_idx, unlabeled_idx = list(range(args.labeled_num)), list(range(args.labeled_num, 100))
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        for i_batch, sample_batch in enumerate(train_dataloader):
            ...
            # 2.4.confident learning loss
            # 2.4.1.uncertainty Estimate
            T = 4 # repeat_times
            img_batch_r = unlabeled_img_batch.repeat(2, 1, 1, 1) # [20, 1, 512, 512]
            stride = unlabeled_img_batch.shape[0] # 10
            ema_outputs = torch.zeros([stride * T, num_classes, patch_size[0], patch_size[1]]).cuda() # [10 * 8, 2, 512, 512]
            for i in range(T // 2):
            ...
            preds = preds.reshape(T, stride, num_classes, patch_size[0], patch_size[1]) # [8, 10, 2, 512, 512]
            ...
            # 2.4.2.confident_maps_np
            if iter_num >= args.iter_clstart:
            ...
            # 2.5.total loss = supervised loss + pseudo loss + pyramid consistency loss + confident loss
            consistency_weight = get_current_consistency_weight(iter_num // 150)
            loss = (supervise_loss1 + supervise_loss2) + consistency_weight * (pseudo_supervision1 + pseudo_supervision2 + consistency_loss1 + consistency_loss2 + args.weak_weight * confident_loss)
            ...
            # 4.writer & loging
            logging.info(f'iteration {iter_num} : total_loss: {loss.item()}; supervise_loss: {supervise_loss1.item()}, {supervise_loss2.item()}; pseudo_supervision: {pseudo_supervision1.item()}, {pseudo_supervision2.item()}; consistency_loss: {consistency_loss1}, {consistency_loss2}; confident_loss: {confident_loss}')
            ...
            writer.add_scalar('loss/consistency_loss/consistency_loss1', consistency_loss1, iter_num)
            writer.add_scalar('loss/consistency_loss/consistency_loss2', consistency_loss2, iter_num)
