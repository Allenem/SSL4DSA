...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.avgpool(x) # [B, C, H/2, W/2]
        # x = x.view(-1, C, H//2*W//2).permute(0, 2, 1)
        x = x.permute(0, 2, 1) # [B, C, L] -> [B, L, C], [B, C, H*W] -> [B, H*W, C]
        for blk in self.blocks:
            x = checkpoint.checkpoint(blk, x) if self.use_checkpoint else blk(x) # [B, H*W, C]
...
class InceptionSwinUnet(nn.Module):
    ...
    def __init__(
        ...
        self.final_blocks = nn.ModuleList([
            nn.Conv2d(embed_dim*4, num_classes, kernel_size=3, padding=1),  
            nn.Conv2d(embed_dim*2, num_classes, kernel_size=3, padding=1), 
            nn.Conv2d(embed_dim, num_classes, kernel_size=3, padding=1),
            nn.Conv2d(embed_dim, num_classes, kernel_size=3, padding=1),
        ])
    ...
    def forward_down(self, x):
        ...
        for layer_down in self.layers_down:
            # append sth. before the next layer for decoder
    ...
    def forward_up(self, x, x_dowmsamples):
        ...
        for idx, layer_up in enumerate(self.layers_up):
            if idx == 0:
                ...
                x = layer_up(x)
            # print(x.shape)
            x_upsample = x.permute(0, 2, 1).view(x.shape[0], x.shape[2], int(sqrt(x.shape[1])), int(sqrt(x.shape[1])))
            # print(x_upsample.shape)
            x_upsamples.append(x_upsample)
        x = self.norm_after_decoder(x)

        if len(x_upsamples) == 4:
            for i in range(3):
                x_upsamples[i] = self.final_blocks[i](x_upsamples[i])
                x_upsamples[i] = nn.Upsample(size=self.shape, mode='bilinear')(x_upsamples[i])
            x_upsamples.pop()
        elif len(x_upsamples) == 3:
            for i in range(3):
                x_upsamples[i] = self.final_blocks[i+1](x_upsamples[i])
                x_upsamples[i] = nn.Upsample(size=self.shape, mode='bilinear')(x_upsamples[i])
        x_upsamples.append(x)
    ...
    def forward(self, x):
        ...
        x_upsamples = self.forward_up(x, x_dowmsamples)
        x = self.forward_final(x_upsamples[-1])

        return [x_u for x_u in x_upsamples[:-1]] + [x]
...
iswin_config_4layers = ml_collections.ConfigDict({
...
if __name__ == '__main__':
    ...
    model = InceptionSwinUnet_from_config(config=iswin_config_3layers)
    ...
    out = model(torch.randn(12, 1, 512, 512))
    if type(out) == torch.Tensor:
        print("out size: {}".format(out.size()))
    elif type(out) == list:
        for i in range(len(out)):
            print("out{} size: {}".format(i, out[i].size()))

    out = model(torch.randn(8, 1, 512, 512))
    if type(out) == torch.Tensor:
        print("out size: {}".format(out.size()))
    elif type(out) == list:
        for i in range(len(out)):
