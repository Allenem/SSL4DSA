...
class Vnet(nn.Module):
    def __init__(self, inChans, outChans, elu=True, nll=False):
        ...
        self.final_blocks = nn.ModuleList([
            nn.Conv2d(256, outChans, kernel_size=3, padding=1),  
            nn.Conv2d(128, outChans, kernel_size=3, padding=1), 
            nn.Conv2d(64, outChans, kernel_size=3, padding=1),
        ])
    ...
    def forward(self, x):
        ...
        de_out   = self.out_tr(de32)
        # return de_out
        # return self.activation(de_out)
        
        outputs_diff_scale = [de256, de128, de64]
        outs = []
