...
class Unet(nn.Module):
    def __init__(self, in_ch=3, out_ch=1, n_base_channels=64):
        ...
        self.final_blocks = nn.ModuleList([
            nn.Conv2d(n_base_channels * 8, out_ch, kernel_size=3, padding=1),  
            nn.Conv2d(n_base_channels * 4, out_ch, kernel_size=3, padding=1), 
            nn.Conv2d(n_base_channels * 2, out_ch, kernel_size=3, padding=1),
            nn.Conv2d(n_base_channels * 1, out_ch, kernel_size=1), 
        ])
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.up_blocks):
            ...
            outputs_diff_scale.append(out)

        # out = self.final_block(out)
        outs = []
        for i, block in enumerate(self.final_blocks):
            out = block(outputs_diff_scale[i])
            if i < len(self.final_blocks) - 1:
                out = nn.Upsample(size=shape, mode='bilinear')(out)
            outs.append(out)
